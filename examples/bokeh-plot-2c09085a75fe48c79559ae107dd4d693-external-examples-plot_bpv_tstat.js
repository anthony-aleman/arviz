(function() {
  var fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      var force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      var element = document.getElementById("efff2761-a1ac-4b15-8406-7d92611f117d");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid 'efff2761-a1ac-4b15-8406-7d92611f117d' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error(url) {
          console.error("failed to load " + url);
        }
    
        for (let i = 0; i < css_urls.length; i++) {
          const url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        const hashes = {"https://cdn.bokeh.org/bokeh/release/bokeh-2.3.3.min.js": "dM3QQsP+wXdHg42wTqW85BjZQdLNNIXqlPw/BgKoExPmTG7ZLML4EGqLMfqHT6ON", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.3.3.min.js": "8x57I4YuIfu8XyZfFo0XVr2WAT8EK4rh/uDe3wF7YuW2FNUSNEpJbsPaB1nJ2fz2", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.3.3.min.js": "3QTqdz9LyAm2i0sG5XTePsHec3UHWwVsrOL68SYRoAXsafvfAyqtQ+h440+qIBhS"};
    
        for (let i = 0; i < js_urls.length; i++) {
          const url = js_urls[i];
          const element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.async = false;
          element.src = url;
          if (url in hashes) {
            element.crossOrigin = "anonymous";
            element.integrity = "sha384-" + hashes[url];
          }
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      var js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.3.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.3.3.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.3.3.min.js"];
      var css_urls = [];
      
    
      var inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            var fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  var docs_json = '{"23fdaeca-e2ab-42e5-90a2-4428c235d7db":{"defs":[],"roots":{"references":[{"attributes":{},"id":"15856","type":"BasicTicker"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"15879","type":"Line"},{"attributes":{"overlay":{"id":"15867"}},"id":"15861","type":"BoxZoomTool"},{"attributes":{"active_multi":null,"tools":[{"id":"15859"},{"id":"15860"},{"id":"15861"},{"id":"15862"},{"id":"15863"},{"id":"15864"},{"id":"15865"},{"id":"15866"}]},"id":"15869","type":"Toolbar"},{"attributes":{},"id":"15894","type":"BasicTickFormatter"},{"attributes":{"fill_color":{"value":"#2a2eec"},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"15905","type":"Circle"},{"attributes":{"items":[{"id":"15903"}]},"id":"15902","type":"Legend"},{"attributes":{"toolbars":[{"id":"15869"}],"tools":[{"id":"15859"},{"id":"15860"},{"id":"15861"},{"id":"15862"},{"id":"15863"},{"id":"15864"},{"id":"15865"},{"id":"15866"}]},"id":"15928","type":"ProxyToolbar"},{"attributes":{},"id":"15852","type":"BasicTicker"},{"attributes":{"children":[{"id":"15929"},{"id":"15927"}]},"id":"15930","type":"Column"},{"attributes":{"toolbar":{"id":"15928"},"toolbar_location":"above"},"id":"15929","type":"ToolbarBox"},{"attributes":{},"id":"15864","type":"UndoTool"},{"attributes":{"source":{"id":"15883"}},"id":"15887","type":"CDSView"},{"attributes":{"callback":null},"id":"15866","type":"HoverTool"},{"attributes":{"children":[[{"id":"15842"},0,0]]},"id":"15927","type":"GridBox"},{"attributes":{"axis":{"id":"15855"},"dimension":1,"ticker":null},"id":"15858","type":"Grid"},{"attributes":{"data":{},"selected":{"id":"15923"},"selection_policy":{"id":"15922"}},"id":"15904","type":"ColumnDataSource"},{"attributes":{},"id":"15897","type":"Selection"},{"attributes":{"formatter":{"id":"15891"},"major_label_policy":{"id":"15892"},"ticker":{"id":"15856"}},"id":"15855","type":"LinearAxis"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"15897"},"selection_policy":{"id":"15896"}},"id":"15878","type":"ColumnDataSource"},{"attributes":{},"id":"15847","type":"LinearScale"},{"attributes":{"data_source":{"id":"15904"},"glyph":{"id":"15905"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"15906"},"view":{"id":"15908"}},"id":"15907","type":"GlyphRenderer"},{"attributes":{"bottom_units":"screen","fill_alpha":0.5,"fill_color":"lightgrey","left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","syncable":false,"top_units":"screen"},"id":"15867","type":"BoxAnnotation"},{"attributes":{},"id":"15896","type":"UnionRenderers"},{"attributes":{"text":"y / y","text_font_size":"15pt"},"id":"15909","type":"Title"},{"attributes":{},"id":"15845","type":"DataRange1d"},{"attributes":{"fill_alpha":0.5,"fill_color":"lightgrey","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"syncable":false,"xs_units":"screen","ys_units":"screen"},"id":"15868","type":"PolyAnnotation"},{"attributes":{"source":{"id":"15904"}},"id":"15908","type":"CDSView"},{"attributes":{},"id":"15862","type":"WheelZoomTool"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#2a2eec"},"line_alpha":{"value":0.1},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"15906","type":"Circle"},{"attributes":{},"id":"15843","type":"DataRange1d"},{"attributes":{},"id":"15899","type":"Selection"},{"attributes":{"axis":{"id":"15851"},"ticker":null},"id":"15854","type":"Grid"},{"attributes":{"data":{},"selected":{"id":"15899"},"selection_policy":{"id":"15898"}},"id":"15883","type":"ColumnDataSource"},{"attributes":{"line_alpha":0,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"15884","type":"Line"},{"attributes":{},"id":"15895","type":"AllLabels"},{"attributes":{},"id":"15898","type":"UnionRenderers"},{"attributes":{"data_source":{"id":"15883"},"glyph":{"id":"15884"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"15885"},"view":{"id":"15887"}},"id":"15886","type":"GlyphRenderer"},{"attributes":{"label":{"value":"bpv=0.53"},"renderers":[{"id":"15886"}]},"id":"15903","type":"LegendItem"},{"attributes":{},"id":"15892","type":"AllLabels"},{"attributes":{},"id":"15923","type":"Selection"},{"attributes":{"data_source":{"id":"15878"},"glyph":{"id":"15879"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"15880"},"view":{"id":"15882"}},"id":"15881","type":"GlyphRenderer"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"15885","type":"Line"},{"attributes":{},"id":"15891","type":"BasicTickFormatter"},{"attributes":{"formatter":{"id":"15894"},"major_label_policy":{"id":"15895"},"ticker":{"id":"15852"}},"id":"15851","type":"LinearAxis"},{"attributes":{},"id":"15922","type":"UnionRenderers"},{"attributes":{},"id":"15860","type":"PanTool"},{"attributes":{"below":[{"id":"15851"}],"center":[{"id":"15854"},{"id":"15858"},{"id":"15902"}],"height":500,"left":[{"id":"15855"}],"output_backend":"webgl","renderers":[{"id":"15881"},{"id":"15886"},{"id":"15907"}],"title":{"id":"15909"},"toolbar":{"id":"15869"},"toolbar_location":null,"width":500,"x_range":{"id":"15843"},"x_scale":{"id":"15847"},"y_range":{"id":"15845"},"y_scale":{"id":"15849"}},"id":"15842","subtype":"Figure","type":"Plot"},{"attributes":{},"id":"15849","type":"LinearScale"},{"attributes":{},"id":"15865","type":"SaveTool"},{"attributes":{"source":{"id":"15878"}},"id":"15882","type":"CDSView"},{"attributes":{"overlay":{"id":"15868"}},"id":"15863","type":"LassoSelectTool"},{"attributes":{},"id":"15859","type":"ResetTool"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"15880","type":"Line"}],"root_ids":["15930"]},"title":"Bokeh Application","version":"2.3.3"}}';
                  var render_items = [{"docid":"23fdaeca-e2ab-42e5-90a2-4428c235d7db","root_ids":["15930"],"roots":{"15930":"efff2761-a1ac-4b15-8406-7d92611f117d"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    var attempts = 0;
                    var timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (var i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();